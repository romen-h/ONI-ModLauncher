...
namespace ONIModLauncher.Core
{
    public class SteamIntegration
    {
		...
		public void Init()
		{
			...
			SteamInstallFolder = FindSteamInstallFolder();
			if (SteamInstallFolder == null) return;

			if (Environment.OSVersion.Platform == PlatformID.Win32NT)
			{
				// steam.exe is in the root folder only on Windows
				SteamExecutablePath = Path.Combine(SteamInstallFolder, "steam.exe");
			}
			else if (Environment.OSVersion.Platform == PlatformID.Unix)
			{
				SteamExecutablePath = "/usr/bin/steam";
			}
			
			if (!File.Exists(SteamExecutablePath))
			{
				InitError = $"Could not find steam executable.";
				return;
			}
			...
			{
				ONIExecutablePath = Path.Combine(ONIFolder, "OxygenNotIncluded");	
			}
		...
		private string? FindSteamInstallFolder()
		{
			switch (Environment.OSVersion.Platform)
			{
				case PlatformID.Win32NT:
					return FindSteamInstallFolderWindows();
				case PlatformID.Unix:
					return FindSteamInstallFolderLinux();
				default:
					InitError = "Unsupported operating system.";
					return null;
			}
		}
		
		private string? FindSteamInstallFolderWindows()
		{
			try
			{
				using (RegistryKey key = Registry.LocalMachine.OpenSubKey("SOFTWARE\\Wow6432Node\\Valve\\Steam"))
				{
					if (key != null)
					{
						string folder = (string)key.GetValue("InstallPath");
		...
		private string? FindSteamInstallFolderLinux()
		{
			string homeFolder = Environment.GetFolderPath(Environment.SpecialFolder.UserProfile);
			
			string steamInstallFolder = Path.Combine(homeFolder, ".steam/steam");
			if (!Directory.Exists(steamInstallFolder))
			{
				InitError = $"Steam install folder doesn't exist: {steamInstallFolder}";
				return null;
			}
			
			return steamInstallFolder;
		}
